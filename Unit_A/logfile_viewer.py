...
"""
# LEGO Robot Log Görselleştirme Aracı

Bu kod, LEGO robot log dosyalarını görselleştirmek ve analiz etmek için geliştirilmiş bir Python uygulamasıdır. 
Uygulama, Tkinter kütüphanesi kullanılarak oluşturulmuş bir grafik arayüzü sunar.

## Özellikler:

- İki görselleştirme paneli: Dünya koordinat sistemi ve sensör koordinat sistemi görünümleri
- Zaman içinde veri adımları arasında gezinmek için interaktif kaydırıcı
- Robotun izlediği yol, tarayıcı verileri ve ortamdaki işaretlerin (silindirlerin) görselleştirilmesi
- Gerçek dünya koordinatları ile canvas koordinatları arasında dönüşüm yapan yardımcı fonksiyonlar
- Birden fazla log dosyasını yükleme ve görselleştirme desteği

## Kullanım:

1. Uygulamayı çalıştırın
2. "Load (additional) logfile" düğmesini kullanarak bir log dosyası seçin
3. Kaydırıcıyı kullanarak robotun zaman içindeki hareketini ve ölçümlerini görüntüleyin
4. İsterseniz "Reload all" düğmesini kullanarak tüm verileri yeniden yükleyin

Bu uygulama, robotik derslerinde ve projelerinde robot konumlandırma ve haritalama verilerini analiz etmek 
...
def load_data():
    ...
    global draw_objects
    draw_objects = []# Python routines to inspect a ikg LEGO robot logfile.
...
class ScannerData(DrawableObject):
    def __init__(self, list_of_scans, canvas, canvas_extents, scanner_range):
        ...
        for s in list_of_scans:
            ...
            for m in s:
                ...
                x = m * cos(angle)
                y = m * sin(angle)
                poly.append(to_sensor_canvas(
                    (x, y), canvas_extents, scanner_range))
                i += 1
            poly.append(to_sensor_canvas(
                (0, 0), canvas_extents, scanner_range))
            self.scan_polygons.append(poly)
    ...
    def draw(self, at_step):
        if self.cursor_object:
            ...
            self.cursor_object = None
        if at_step < len(self.scan_polygons):
            self.cursor_object = self.canvas.create_polygon(
                self.scan_polygons[at_step], fill="blue")
    ...
    def background_draw(self):
        for l in self.landmarks:
            if l[0] == 'C':
                x, y = l[1:3]
                ll = to_world_canvas(
                    (x - l[3], y - l[3]), self.canvas_extents, self.world_extents)
                ur = to_world_canvas(
                    (x + l[3], y + l[3]), self.canvas_extents, self.world_extents)
                self.canvas.create_oval(
                    ll[0], ll[1], ur[0], ur[1], fill=self.color)
...
def load_data():
    ...
    # Insert: landmarks.
    draw_objects.append(Landmarks(logfile.landmarks,
                        world_canvas, canvas_extents, world_extents))
    ...
    if logfile.filtered_positions:
        if len(logfile.filtered_positions[0]) > 2:
            positions = [tuple(list(to_world_canvas(
                pos, canvas_extents, world_extents)) + [pos[2]]) for pos in logfile.filtered_positions]
        else:
            positions = [to_world_canvas(
                pos, canvas_extents, world_extents) for pos in logfile.filtered_positions]
        draw_objects.append(Trajectory(positions, world_canvas,
...
if __name__ == '__main__':
    ...
    frame1.pack()
    world_canvas = Canvas(
        frame1, width=canvas_extents[0], height=canvas_extents[1], bg="white")
    world_canvas.pack(side=LEFT)
    sensor_canvas = Canvas(
        frame1, width=sensor_canvas_extents[0], height=sensor_canvas_extents[1], bg="white")
